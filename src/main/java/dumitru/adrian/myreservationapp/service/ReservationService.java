...
//            Long restaurant_id = restaurant.getId();
            ...
            List<Reservation> reservations = reservationRepository.findAllBetweenDates(ReservationUtil.convertDateToString(reservation_start_hour),
                ReservationUtil.convertDateToString(reservation_end_hour),reservation.getRestaurant().getId().toString());
            ...
            List<Reservation> reservations = reservationRepository.findAllByRestaurantIdAndDay(reservation.getRestaurant().getId(),reservation.getDay());
            Integer free_tables;
            free_tables = reservation.getRestaurant().getRestaurant_tables().getFour_persons_table();
            if(reservations.size() != 0){
                for(Reservation current_reservation : reservations)
                    free_tables = free_tables - current_reservation.getTables();
            }

            if(free_tables >= reservation.getTables())
                reservationRepository.save(reservation);
            else
                throw new Exception("NOT_ENOUGH_TABLES");
        }
